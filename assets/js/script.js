...
// GET AND DISPLAY PREVIOUS CITIES FROM LOCAL STORAGE    
    function showPrevCities(type) {
        ...
        for (let k = 0; k < prevCities.length; k++) {
            // if(type=="add"){
            //     k = prevCities.length-1;
            //     console.log("type: "+ type + "k=" + k);
            // };
...
function storeCity(lat,lon,city,state) {
    ...
    citiesArray.unshift({ city: city, state: state, lat: lat, lon: lon}); // stick new city on teh end (TO-DO: Change to UNSHIFT)
    // citiesArray.unshift({ city: city, state: state, lat: lat, lon: lon});
...
//GET THE WEATHER LOWDOWN
    function getWeather(lat,lon,city,state) {
      ...
      function drawWeather(d,city,state) {
        console.log("Hello from drawWeather");
        console.log(d);
...
// DISPLAY THE 5 DAY FORECAST   
        ...
        // lowE.setAttribute("class","fiveDayCard");
        // highE.setAttribute("class","fiveDayCard");
        // oneDay.setAttribute("style","width: 10rem")
